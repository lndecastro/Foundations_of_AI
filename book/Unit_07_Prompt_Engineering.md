...
A prompt library is a small set of prompts you have refined and can reuse. Professionals keep one. Building it is HW7.

```python
library = {
    "code_review": {
        "role": "Senior engineer reviewing a pull request.",
        "task": "Identify correctness bugs in the code below.",
        "constraints": "Ignore formatting and naming. Flag only issues that "
                       "could produce incorrect behavior at runtime.",
        "format": "Table: line | issue | why it breaks | minimal fix",
        "criteria": "If there are no correctness bugs, say so and stop.",
        "verify": "Reproduce each claimed bug with a test before believing it.",
    },
    "explain_unfamiliar_code": {
        "role": "Engineer onboarding a new teammate.",
        "task": "Explain what this function does and why it exists.",
        "constraints": "Assume the reader knows the language but not this codebase.",
        "format": "One paragraph of purpose, then a bulleted walkthrough.",
        "criteria": "Name any assumption the code makes about its inputs.",
        "verify": "Check the walkthrough against the code line by line.",
    },
    # ADD YOUR OWN — at least five, from tasks you actually repeat
}

def render(entry):
    order = ["role", "task", "context", "constraints", "format", "criteria"]
    return "\n".join(entry[k] for k in order if k in entry)
...
Note that every entry carries a `verify` field. A prompt without a verification step is half a tool.
