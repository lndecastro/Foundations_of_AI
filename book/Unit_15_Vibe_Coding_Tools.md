...
So the tutorials are comparable, every group attempts the same benchmark with its assigned tool:

> Build a small web page that loads a CSV of incident records (`date`, `severity`, `minutes_to_resolve`), shows resolution-time distribution by severity, and flags any severity level whose median exceeds a configurable threshold.

Report: number of prompts to something runnable, number of prompts to something *correct*, what it assumed you never specified, and what broke first.
